...
export class ChartCardLineComponent implements OnChanges {
    ...
    ngOnChanges(): void{
        ...
        const hidden: Map<string, boolean> = this.getHiddenStatuses();
     ...
     */
    sampleChartData(statuses: Map<string, boolean>): void{
         ...
         this.displayChartData.chartDataSet.forEach(set => {
            ...
            for (const [index, point] of set.data.entries()){
     ...
     */
    sampleChartLabels(): void{

        // Reduce Labels based on sampling specified
        const reducedLabels: Label[] = [];
        for (const [index, label] of this.displayChartData.chartLabels.entries()){
    ...
    getHiddenStatuses(): Map<string, boolean>{
        ...
        const sets: Map<string, boolean> = new Map();
        ...
        if (this.chart && this.chart.datasets){
            ...
            labels.forEach((label, index) => {
                ...
                const hidden = this.chart.isDatasetHidden(index) !== null ?
